<!--
  This is the upstream `agentic-dev-boilerplate` documentation — the framework Fresco is
  built on. It was the repo's original README before Fresco had its own (FRESCO-316).
  For the Fresco product and how to run this app, see the root README.md.
  Keep this file when running `bun up` so the boilerplate's own overview stays available.
-->
...
| **Methodology / philosophy / extension guide** (~25 min)           | [`docs/agentic-development-engineering.md`](agentic-development-engineering.md)                                                                                                                    |
