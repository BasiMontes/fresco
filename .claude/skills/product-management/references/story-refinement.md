...
- [ ] **Testability gate passed** — for every scenario, answer YES to *"could a tester write the test that fails from this `Then` alone, without asking me anything?"* Each `Then` names an observable value (exact count, exact message text, exact final state), never a qualifier. The zero / empty / error case is its own scenario. A scenario that fails this is a `gap` — resolve the value with the PO before the story enters a sprint, never pass it as-is (anti-pattern `I22`).
...
- [ ] **Story Points** — leave `{{jira.story_points}}` EMPTY by default. Populate ONLY if the user explicitly requested estimation in this session. Estimation belongs to the team that will build the story (Design + Dev + Test), not to the PO/BA. When opted-in: Fibonacci (1, 2, 3, 5, 8); 13+ → split. See anti-pattern `I16`. The field gets its real value later, from the **dev team at `/sprint-development` Stage 1** (`references/story-plan.md` § Estimated Effort) — that is where the FRESCO-281 structured-QA-field contract is satisfied, not here.
