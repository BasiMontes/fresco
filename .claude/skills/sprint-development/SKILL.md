...
Review notes live **in the PR** — the adjudication verdict table goes in the PR description (or a single review comment), and the QA-handoff summary goes in the Jira `Ready For QA` comment (Stage 4). No separate `review.md` file is written under `.context/PBI/` (retired 2026-08-28, FRESCO-290 — the local artifact duplicated the PR + Jira record and nobody read it). A human-prompted architectural decision that came out of review still goes to `.context/ADR/` per the ADR doctrine, not a review file.

**Adversarial review + adjudication (orchestrator-owned).** The reviewer is dispatched as an INDEPENDENT, adversarial agent (fresh context, no stake in the implementation) and returns severity-tagged findings (`BLOCKER | MAJOR | MINOR | NIT`) with `file:line` evidence — it proposes, it does not apply. The orchestrator then **adjudicates each finding** instead of auto-accepting: verify it against the actual diff + AC, mark `legitimate` (apply) or `false-positive` (dismiss WITH a one-line reason). Only `legitimate` findings loop back to Stage 2 via `fix-issues.md`. Record the per-finding verdict table in the PR (description or a single review comment). This prevents BOTH rubber-stamping AND blindly churning on reviewer noise. In SOLO mode the orchestrator runs a deliberate fresh-eyes review pass inline, then adjudicates the same way. Architectural rework loops back to Stage 1 with a new spec (rare). See the adjudication contract in `references/review-pr.md`.
...
The matrix lands **in the PR description as a required section** — no separate `compliance-matrix.md` file under `.context/PBI/` (retired 2026-08-28, FRESCO-290, alongside `review.md`). Algorithm, four `covered_by` shapes with examples, full status legend, and the 2FA-login worked example: see `references/spec-compliance-matrix.md`.

**Regression-suite automation (Gherkin AC stories — FRESCO-321, Gotcha 16).** If this Story added scenarios to `.context/qa/regression.feature`, each of those scenario rows in the matrix defaults to `covered_by: test:<tests/steps/*.ts>` — the step file is in THIS PR and `bun run test:e2e` is green in CI. A Gherkin AC scenario row that is `manual:` or `exempt:` carries a one-line reason. Apply the ADR-0014 budget clause from Gotcha 16 (~60 `@automatizado` / ~8 min `test:e2e` ceiling): over budget → automate the happy path, mark edge-case rows `manual:<reason — ADR-0014 budget>`, say so in the PR. Tag the promoted scenarios `@automatizado` with the spec-file comment per `.context/qa/README.md` § Lifecycle; never delete the Gherkin text.
...
10. **Test automation scope in this skill**: unit tests live in Stage 2 via `/unit-testing`. Standing up E2E / integration *infrastructure* is out of scope, and QA verification on staging is out of scope. **One carve-out (FRESCO-321)**: when a Story's AC ship as Gherkin scenarios added to `.context/qa/regression.feature`, those scenarios get automated (`@automatizado` tag + a `tests/steps/*.ts` step file, per ADR-0014's `playwright-bdd` setup) **inside the same PR** — see Gotcha 16.
...
15. **Reproduction-or-rejection gate** (FRESCO-313): a defect does not reach any terminal status (`Finalizada` / `Rechazos` / `Won't Fix` / `Duplicate`) without EITHER numbered reproduction steps OR a documented reason it cannot/should not be reproduced (env difference, WAD with code cited, duplicate link, deferred rationale). Agent-side reinforcement of the Jira workflow validator the owner configures on the Error workflow — apply even where the validator is not yet in place. See `references/bug-fix-workflow.md` Phase 3 gate + Phase 7 confirm.
16. **Gherkin AC → automated in the same PR** (FRESCO-321, eje Verificación): when a Story ships new Gherkin scenarios in `.context/qa/regression.feature`, its Stage 3 Spec Compliance Matrix rows for those scenarios default to `covered_by: test:<step-file>` — the scenario is tagged `@automatizado` and a `tests/steps/*.ts` step file lands in the SAME PR (`bun run test:e2e` green). `manual:` / `exempt:` for a Gherkin AC scenario needs a one-line reason in the matrix. **Budget clause (ADR-0014)**: if adding these steps would push the `@automatizado` count past **~60** or the `test:e2e` job past **~8 min**, automate the happy-path scenario only, mark the edge-case rows `manual:<reason — ADR-0014 budget>`, and note it in the PR — do not silently skip. The `@smoke` subset is owned by `.context/qa/README.md` + FRESCO-322, not chosen here. Ratchet target + core-flow list live in `.context/qa/README.md` § Automation ratchet.
17. **Definition of Done — close on the metric, not the mechanism** (FRESCO-404): a ticket does not reach `Finalizada` until the problem that created it is measurably zero — the AC-covering test passes / the verification query returns 0 / the defect's finding is gone — unless the remaining work is a **named, ticketed, concrete** follow-up (`Relates`-linked). A deferral goes to `Rechazos` with an ADR or process-note pointer, never `Finalizada`. Gotchas 13–15 are the defect-specific instance. Full policy: `.context/backlog/definition-of-done.md`.
...
- **S18.** NEVER transition a defect to a terminal status (`Finalizada` / `Rechazos` / `Won't Fix` / `Duplicate`) with neither reproduction steps nor a documented rejection rationale. A bare one-line defect is not closable — add the missing half first (FRESCO-313).
- **S19.** NEVER merge a Story that added Gherkin scenarios to `.context/qa/regression.feature` while leaving those scenarios `manual:` with no reason — the default is `@automatizado` + a step file in the same PR (Gotcha 16). Deferring is allowed only when the ADR-0014 budget clause applies AND the matrix row says so (`manual:<reason — ADR-0014 budget>`). Silent "automate it later" is how the ratio decayed to ~23% in the first place (FRESCO-321).
