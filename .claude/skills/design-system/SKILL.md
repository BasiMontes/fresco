...
- **(Optional, opt-in)** `BRIEF.md` per batch at `.context/designs/<project>/<batch-slug>/` — portable
  design brief the user pastes into the external tool when mockups don't exist yet. Generated by
  `references/screen-design-brief.md`; doubles as provenance for the returned bundle.
