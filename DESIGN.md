...
The palette is two hand-picked corporate hues — "verde corporativo" and "naranja corporativo" — set against warm, cream-toned neutrals rather than a gray scale. Nothing here is a stock Tailwind or shadcn default; every hex was chosen on the brand canvas and is preserved verbatim. The frontmatter `colors` block is the **light** palette; the **dark** palette (FRESCO-448) is a role-token + ramp swap defined in `app/globals.css`, not here — see §Dark mode.
...
- **Photography is the colour budget.** Terracotta, olive, ceramic, char — those come from the dish, on a near-neutral UI. The unified crop + grade that keeps ~600 stock photos reading as one brand is specified in §Photography.
...
**The role rule (v2):** Fraunces appears on `h1` and `h2` ONLY. `h3`–`h6` are Figtree 600. Card titles (including recipe cards) are Figtree, capped at 2 lines (`line-clamp: 2`) so the title never competes with the photo. Button labels are Figtree 600. If a heading is not a page-level or section-level headline, it is Figtree. **One sanctioned exception (FRESCO-445):** the landing footer closes on an oversized Fraunces wordmark ("Fresco", ≥60px, weight 400) used as a *graphic element*, not a heading — the editorial signature. The landing hero uses real recipe photography for its product imagery, never illustrative emoji.
...
**v2 (FRESCO-447) — one crop, one grade.** The recipe catalogue is ~600 photos sourced from ~600 places (Unsplash, Pexels, Pixabay — FRESCO-31). Making them read as one brand is the main compensator for not buying original photography. Two rules, both applied as shared code, never per screen:

- **One crop ratio.** Every recipe photo area is `aspect-[4/3]` with `object-fit: cover` — `/menu`, `/calendar`, `/recipes`, `/favorites` (all via `components/recipe/recipe-card-media.tsx`) and the `/recipes/[id]` detail hero. No surface uses a different ratio. `cover` never stretches; a focal point would be honoured if the pipeline produced one (it does not today — centred crop).
- **One unifying grade.** The `.recipe-photo` utility in `app/globals.css` is the single filter: a light warm grade (`saturate(0.92) contrast(1.04) sepia(0.08) brightness(1.01)`) so photos shot under different light land on a shared tone. It is applied to the real `<img>` only — never to the `RecipePlaceholder` (a designed per-category gradient) and never re-declared in a component's own CSS.
- **No-photo state.** `components/recipe/recipe-placeholder.tsx` — the category gradient + typographic initial. Same on cards and on the detail view (FRESCO-441 + FRESCO-447); never a bare line-icon on beige.
- **Out of scope here.** The landing hero composition (`components/landing/hero.tsx`) is a hand-curated set ratified under FRESCO-445 and is not run through `.recipe-photo`.
...
- **What gets motion (v2 additions, FRESCO-446 — shipped).** Landing: gentle section reveals on scroll — `components/ui/reveal.tsx` (`IntersectionObserver`, wraps every `<main>` section except the above-the-fold hero), driven by the `[data-reveal]` rule in `app/globals.css` (`--duration-slow` / `--distance-medium` / `--ease-smooth-out`). App: the existing hover/press, dropdown/sheet, tab and page transitions stay unchanged; the `card-insight` (`learning_explanation_card`, `/menu`) gets a one-shot `[data-insight-enter]` fade-rise on render. Both gated by `@media (prefers-reduced-motion: no-preference)` so the content is plain-visible under reduced motion with no JS dependency. No new tokens, no spring. Not: decorative loops, attention pulses, or anything on the path of reading a recipe.

## Dark mode

**v2 (FRESCO-448) — an EXTENSION of the committed light look, not a second design.** v1/v2 were authored as a single light "warm cream editorial" world. Dark mode keeps every part of the thesis — *contención*, hairline-first surfaces, one accent = CTA, calm typography, the green+orange brand — and only re-grounds the paper: from warm cream onto a deep green-black anchored on the `accent` (green) ramp's dark end. It is not a redesign and it does not get its own component rules; a component that follows the token discipline below is already correct in both themes.

**Mechanism.** `<html data-theme="light|dark">` is stamped **server-side** in `app/layout.tsx` from a `theme` cookie (`light` | `dark` | `system`) — `dynamic = 'force-dynamic'` already gives every render a request context, so the right palette paints on the first byte with no flash and no inline script. `system` (and first visit, no cookie) emits **no** attribute and falls through to `@media (prefers-color-scheme: dark)`. The preference control is `components/theme/theme-toggle.tsx` (a three-segment radio: claro / automático / oscuro), placed in the desktop sidebar footer and — because the sidebar does not exist on mobile — as an "Apariencia" row on `/profile`. It writes the cookie and mirrors `data-theme` onto `<html>` for an instant swap; `lib/theme/theme.ts` owns that contract.

**Token strategy — swap roles + ramps, never per-component `dark:`.** Every themed token in `app/globals.css` is defined **once** as `light-dark(<light>, <dark>)` on `:root`. Two rules flip `:root`'s `color-scheme` — which is what `light-dark()` reads: `dark` under `:root[data-theme='dark']` (the toggle) or `@media (prefers-color-scheme: dark)` when not forced back to light. No duplicated dark block to keep in sync. (The `[data-brand-ground]` sidebar subtree re-declares the handful of role tokens with static light values — `light-dark()` resolves against `:root`'s color-scheme, not a subtree's, so a nested `color-scheme` alone would not pin it.) Two moves:

- **Role tokens** are re-pointed: `background #011101` (accent-900), `surface #04180a` (recessed), `surface-raised #0c2712` (cards), `text #f2e8d2` (warm cream), `border` = cream at **30%** (FRESCO-481: was 15% — a `surface-raised` card measured only 1,22:1 against the near-black page, so the border has to carry that ~3:1 separation; light side stays at 16%), `primary #63b85e` (FRESCO-481: `#1c6b1a` measured 2,93:1 as `text-primary` on the ground and as the button fill vs the page — below AA; raised to the inverted `accent-700` step, ~9:1 on background / ~7:1 on surfaces, and it lifts every `text-primary` chip on `bg-accent-100/200` too), `on-brand #04180a` (FRESCO-481: near-black, since the primary fill is now a LIGHT green — cream on it was ~2:1; light theme keeps cream), `error #e06a56` / `success #4fae4b` (lifted for legibility). `secondary` (`#DF8C26`) and its text pairing (`{colors.text}`) are **unchanged** — orange already pops on dark; the one exception is a saturated-fill *badge* on amber (e.g. pricing "Popular"), which pairs with `on-warning` near-black, not `text`.
- **The three ramps (`accent`, `accent-2`, `neutral`) are inverted end-for-end.** Step `-100` still means "the subtlest tint" and `-900` "the strongest" — they just now resolve to dark values. This is what lets every existing `bg-x-100 / text-x-800` pair keep working untouched: both sides of the pair flip together. `neutral-600` (the FRESCO-443 input-border / switch-OFF-track step) still clears 3:1 on its adjacent surface in dark.
- **Shadows** deepen toward true black (`--shadow-*` tokens, consumed by `tailwind.config.ts`) — a warm-neutral shadow is invisible on a near-black ground; the hairline still does the primary separation work.
- **`color-scheme`** is pinned (`light` on `:root`, `dark` in the dark blocks) so native affordances — autofill, scrollbars, number spinners, unstyled `<textarea>` — match the theme.

**Rule.** New UI uses the semantic tokens (`bg-background`, `bg-surface-raised`, `text-text`, `border-border`, `text-tertiary`, `shadow-sm`) or a ramp step — never a hardcoded hex, never a raw `dark:` colour. If a placement genuinely needs a theme-specific value (rare — e.g. the sidebar's cream-on-green overrides), it must satisfy **both** `@media (prefers-color-scheme: dark)` and `[data-theme="dark"]` (the `darkMode` variant in `tailwind.config.ts` is wired for exactly this).
...
- `input`: pill-shaped (`rounded.full`), `{colors.surface}` background, a **1px `{colors.neutral-600}` border at rest** (FRESCO-443: was the `{colors.border}` text-at-16% hairline, which measures ~1.2:1 on the cream page — below WCAG 2.2 SC 1.4.11's 3:1 for a UI-component boundary; `neutral-600` keeps the 1px width and clears 3:1 on page, card, and the field's own `surface` fill — see master-design-plan §5-N), and a visible **focus ring** in `{colors.primary}` + `ring-offset-2` on focus (v1 had no designed focus state). Placeholder text at `{colors.tertiary}`, not lighter — it must read as an active field, never disabled. Text fields never sit directly on page background. `dropdown` (the custom listbox trigger) uses the same border + focus-ring contract.
...
- ON track: filled `{colors.primary}` (~9:1 on the `surface-raised` card).
- OFF track: `{colors.neutral-600}` fill **plus** a `{colors.border}` hairline. WCAG 2.2 SC 1.4.11 requires **each** track state to clear 3:1 against the adjacent surface *independently* — the ON↔OFF luminance delta does not substitute for it. `neutral-600` measures ~4.3:1 on the `surface-raised` card; `neutral-300` (an earlier draft) was only ~1.4:1 and failed. Never a translucent `{colors.tertiary}` wash (the opacity modifier collapses to transparent on the hex-valued token, leaving no track at all).
...
- `neutral-600` is a warm mid-khaki, not green — the OFF state still reads unmistakably as "off", distinct from the primary-green ON fill and the knob position.
...
- `recipe-card`: **v2 direction (FRESCO-441 owns the redesign; contract here):** the photo leads — a full-width image area at the top (`rounded.image`, `aspect-[4/3]` + `object-fit: cover` + the `.recipe-photo` grade — see §Photography), title below in Figtree capped at 2 lines, meta and a single tag as secondary info, favourite `button-icon` top-right over the photo. The no-photo state is a designed placeholder (subtle monochrome gradient keyed to meal category, or a typographic initial) — never a lone line-icon on beige. Same component and element order on `/menu`, `/calendar`, `/recipes`.
