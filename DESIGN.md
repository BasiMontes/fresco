...
- **Role tokens** are re-pointed: `background #011101` (accent-900), `surface #04180a` (recessed), `surface-raised #0c2712` (cards), `text #f2e8d2` (warm cream), `border` = cream at 15%, `primary #1c6b1a` (lifted one notch from `#0f4e0e` so the CTA still reads on near-black), `error #e06a56` / `success #4fae4b` (lifted for legibility). `secondary` (`#DF8C26`) and its text pairing (`{colors.text}`) are **unchanged** — orange already pops on dark.
