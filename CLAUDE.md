...
- `bun run jira:sync-issues get <KEY>` → lee la historia y TODOS los campos personalizados. **NOTA CRÍTICA**: NO uses el flag `--include-comments` por defecto. Los comentarios históricos consumen tokens masivos y rara vez son relevantes para la implementación. 
- **EXCEPCIÓN**: Usa `bun run jira:sync-issues get <KEY> --include-comments` SOLO si el usuario pregunta explícitamente por el historial de discusión o hay un bloqueo que requiere revisar comentarios antiguos.
