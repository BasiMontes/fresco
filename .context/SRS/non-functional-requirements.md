...
**NFR-PERF-1 — Menu generation completes in under 30 seconds, end-to-end.**
...
- **Scope clarification (resolving an ambiguity between those two source statements):** the founder's own Edge Function implementation (`fresco-edge-function-generate.md`) treats menu generation and shopping-list generation as **two separate, independently invoked Edge Functions** — `generate-meal-plan` runs at the end of onboarding / on regeneration; `generate-shopping-list` is invoked later, on demand, "right after generating the menu, or when the user taps 'View shopping list' for the first time." Given that implementation reality, this NFR reads the 30-second promise as applying to the **menu-generation call alone**: request → user-profile load → SQL pre-filter → deterministic slot selection (`menu-selector.ts`) → persist to `meal_plans`/`meal_plan_recipes` → response. The Executive Summary's combined "menu + shopping list" phrasing is read as the *pitch-level* promise, not a single technical SLA spanning two independently-triggered backend calls.
- **Updated per `ADR-0005` (FRESCO-302, 2026-08-29):** the 30-second budget was sized around a Gemini Flash `generateContent` call (a thinking model, ~20–110s observed) plus up to `MAX_RETRIES = 2` retries and output validation. That call was removed 2026-08-01; slot selection is now a sub-second in-process algorithm with no retries. The 30-second figure is retained as the stated upper bound for the user-facing promise (`mvp-scope.md` US 2.2), but the system now beats it by roughly two orders of magnitude — it is a comfortable ceiling, not a tight p95 SLA.

**NFR-PERF-2 — Shopping-list generation.**
- `[PLACEHOLDER]` — no source document states an explicit latency target for `generate-shopping-list` alone. Since the 2026-08-01 deterministic rewrite (`ADR-0005`) it is a fully in-process pipeline — ingredient consolidation (`consolidator.ts`) plus a static aisle/price map (`aisle-pricing.ts`), no model call — so it should run in low double-digit milliseconds. Set a numeric target from real measurement once baseline traffic exists; there is no model-latency component to budget for.
...
**NFR-SEC-3 — No third-party AI/API credential is present in the system.** *(updated per `ADR-0005`, FRESCO-302, 2026-08-29)*
- Originally: the Gemini API key (`GEMINI_API_KEY`) had to stay a server-only Supabase Edge Function secret, read via `Deno.env.get()` inside the Edge Function runtime, never exposed to the client. Gemini was removed from production on 2026-08-01 (`ADR-0005`); `GEMINI_API_KEY` is no longer read anywhere and has been removed from the Edge Function secret store.
- The general rule stands for any future server-only secret: read via `Deno.env.get()` in the Edge Function runtime only. `SUPABASE_URL` / `SUPABASE_ANON_KEY` are injected automatically and are the only credentials safe to reach the browser.

**NFR-SEC-4 — Allergen and dietary-restriction safety is a reliability-critical NFR, not merely a functional feature.**
- This restates FR-8.1/FR-8.2 as a non-functional guarantee: the SQL pre-filter (`get_filtered_recipes()`, fail-closed — the sole structural enforcement point since the Gemini prompt-rule layer was removed, `ADR-0005`) must maintain a zero-tolerance error budget for allergen leakage. Unlike ordinary feature bugs, a single allergen-leakage incident is a trust and safety failure, not a UX defect — `market-context.md` (Risks: "Food-safety risk (allergies / dietary restrictions handled incorrectly)") names this explicitly as a top-tier product risk, mitigated during the concierge phase by the manual checklist (FR-8.3) precisely because code-level enforcement alone is not yet trusted as sufficient.

**NFR-SEC-5 — Standard input validation and transport security.**
- Server-side validation on every Edge Function input (request body shape, required fields) before any DB write — already present in each Edge Function's early-return error handling. HTTPS/TLS in transit is inherited from the Vercel + Supabase platforms; no source document calls for anything beyond platform defaults, and none is invented here.
...
**NFR-REL-1 — Menu and shopping-list generation are deterministic and cannot produce a partial or malformed result; neither has a retry loop.** *(updated per `ADR-0005`, FRESCO-302, 2026-08-29)*
- Originally both Edge Functions retried up to `MAX_RETRIES = 2` on invalid/malformed Gemini output (`generate-meal-plan`: JSON-parse or validator failure; `generate-shopping-list`: JSON-parse failure, missing `pasillos`, or < 90% ingredient-count retention). Since the 2026-08-01 deterministic rewrite (`ADR-0005`) there is no model output to be malformed: `generate-meal-plan` fills every slot from the pre-filtered catalog by construction (a slot with no safe candidate becomes `NO_SAFE_RECIPE_SENTINEL` + a templated `advertencias` entry, not a failure), and `generate-shopping-list` classifies every consolidated ingredient deterministically (unrecognized → "Otros").
- The reliability contract that remains: a menu or list is never persisted partially — `generate-meal-plan` returns `422` when fewer than 21 recipes survive the SQL pre-filter; `generate-shopping-list` returns `422` when consolidation yields zero items. The former `502` "upstream/LLM failure" path no longer exists in either function.
...
**NFR-A11Y-2 — One documented, scoped exception is inherited from `DESIGN.md`, not newly introduced here:** the `button-action` component (orange fill, cream/white text) measures ~2.4:1 contrast — below the 4.5:1 AA minimum for normal text. `DESIGN.md` explicitly scopes this bypass to short, heading-font, all-caps-weight CTA labels (the "Generar mi menú" pattern, one per screen) and explicitly forbids extending it to body copy or any text longer than two-to-three words. This SRS carries that constraint forward as a hard boundary: any implementation reusing the `button-action` treatment for longer text is an accessibility regression, not a legitimate reuse of an approved exception.
