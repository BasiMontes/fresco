...
## 2026-09-03 - FRESCO-417 follow-up: aceptados los 4 major bumps de acciones
- Qué: Dependabot (#273, primer PR del dependabot.yml) agrupó 4 bumps mayores de las acciones recién ancladas: actions/checkout v4.4.0->v7.0.1, actions/cache v4.3.0->v6.1.0, actions/upload-artifact v4.6.2->v7.0.1, supabase/setup-cli v1.7.1->v3.0.0. Verificados (uso de la API sin cambios; setup-cli v3 respeta el input version:; verde en pr-check + migration-drift-check + edge-functions-drift con setup-cli v3 instalando 2.109.1). PR #275 (rehecho sobre dev) squash -> dev ca75ba8 -> ff staging -> ff main. dependabot.yml ahora con target-branch: dev (#273 apuntaba a main). #273 y #186 (regen vieja de api-map, del 2026-08-29) cerradas.
- Por qué: #273 apuntaba a main saltándose el flujo dev->staging->main y eran majors sin revisar; el AC de 417 exigía "sin cambio de comportamiento".
- Siguiente: los próximos PRs de Dependabot llegarán a dev; revisar antes de promover.
## 2026-09-03 - FRESCO-422: template del email "Reset your password" alineado con el de confirmación
- Qué: `supabase/templates/recovery.html` reconstruido desde `confirmation.html` (head/@font-face/style/tablas/logo byte-idénticos; solo cambian copy + link CTA, link preservado literal). Asunto ahora en español ("Restablece tu contraseña en Fresco"). Aplicado al dashboard de Supabase vía Management API PATCH /v1/projects/jdqemhewjrjuopssdurn/config/auth (los 3 entornos a la vez) - verificado live == repo. + confirmation.html commiteado como referencia + supabase/templates/README.md + .prettierignore. PR #276 squash -> dev de5d8a1 -> ff staging -> ff main.
- Por qué: el email de recovery era un rediseño a medias (div suelto sin head, sin @font-face -> Figtree/Caprasimo no cargaban, asunto en inglés, color-mix() no soportado). El de confirmación (DEFECT-253/254/264) sí tenía el tratamiento completo.
- Siguiente: follow-ups no ticketizados - wire supabase config push + commitear los stubs email_change/magic/invite; arreglar el fragmento MSO huérfano en confirmation.html.

## 2026-09-03 - FRESCO-424: manual de marca de nivel agencia (docs/brand/index.html)
- Qué: fichero HTML nuevo, autocontenido, 13 secciones (logo con área de protección + galería de usos incorrectos, color con tabla de contraste WCAG de 14 pairings, tipografía, voz, movimiento, aplicaciones). Presentacional: no sustituye a DESIGN.md ni al canvas brand-guide.dc.html. Logos SVG inline (fuente exacta), imprimible a PDF, favicon crema como app/icon.svg. PR #277 squash -> dev 47474b8 -> ff staging -> ff main. Jira Finalizada, SP 5.
- Por qué: la marca vivía repartida entre DESIGN.md, el canvas y logos sueltos; faltaba un manual navegable de referencia (nivel brandingstyleguides.com / Frontify).
- Siguiente: opcional wire de /brand como ruta servida por Next; pipeline de regeneración desde DESIGN.md. Flag: supabase/templates/{confirmation,recovery}.html usan voseo (\"confirmá\") — choca con español de España, candidato a ticket. Nuevas historias creadas: FRESCO-425 (histórico de menús, hijo de FRESCO-330) y FRESCO-426 (girar eje tabla de planificación).

## 2026-09-03 - FRESCO-426: girar eje de la tabla de planificacion
- Que: planning-selection-grid.tsx (onboarding paso 3 + perfil Preferencias) - dias pasan a columnas (7), tipos de comida a filas (3). setColumn(comida)->setDay(dia), aria-label "{comida} {dia completo}", controles Todos/Ninguno por dia ocultos en movil (hidden sm:table-row). Modelo planning_selection, generacion de menu y steps e2e intactos. Test de componente nuevo (7). PR #278 squash -> dev 29c19bc -> ff staging -> ff main. Jira Finalizada, SP 2.
- Por que: peticion de usuario - un planificador semanal se lee de izquierda a derecha como un calendario; 3 columnas desperdiciaban el ancho.
- Siguiente: DEPLOY PROD PENDIENTE - el push a main no genero deploy de Produccion en Vercel (solo Preview), casi seguro el cap de 100 deploys/dia del plan free (dia muy cargado: 2 PRs con CI completo + rebuilds de favicon + 3x propagacion dev/staging/main). El codigo esta en main; fresco-pro sigue sirviendo el build de FRESCO-424 (lwhd1l7c3). Se desplegara al resetear el cap (~24h) + siguiente push o "vercel --prod" manual.

## 2026-09-03 - FRESCO-425a + 427: Historico de menus (split de FRESCO-425)
