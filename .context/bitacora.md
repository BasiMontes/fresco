...
## 2026-08-26 - FRESCO-266: primer pipeline de CI (GitHub Actions)
- Que: `.github/` no existia en el repo — nunca hubo CI (Dojo audit, unico hallazgo Alto). Creado `.github/workflows/pr-check.yml`: gate en PRs contra dev/staging/main. Job `repo-check` (format+lint+types+vars+skills) bloqueante. Job `e2e` (test:e2e) informativo (`continue-on-error`) hasta recrear los usuarios de prueba de Supabase (borrados, aun pendiente). Decisiones confirmadas con el usuario antes de codear: trigger branches dev+staging+main (no solo staging+main como decia el ticket, porque el flujo real de PRs aterriza en dev) y sin step de deploy a Vercel (ya lo maneja la integracion nativa de Vercel, fuera de GitHub Actions). PR #146 contra dev, primer run real de CI: repo-check verde, e2e fallo como se esperaba (no bloqueo). Mergeado y propagado dev -> staging -> main (ff-only), ambos deploys de Vercel en Ready. Jira: Listo -> WIP -> Control de calidad -> Merged -> Finalizada.
- Por que: usuario pidio trabajar la 266; hallazgo de auditoria externa con retorno compuesto (cada dia sin CI crecia el volumen de PRs sin proteger).
- Siguiente: agregar secreto `ENV_FILE` en GitHub (Settings -> Secrets and variables -> Actions) con el contenido completo del `.env` local; activar "Require status checks to pass" para `repo-check` en dev/staging/main branch protection; recrear usuarios de prueba de Supabase y flipear `e2e` a bloqueante.

## 2026-08-26 - FRESCO-270: ADR-0014, seguimos en playwright-bdd (no migrar a KATA)
- Que: la auditoria Dojo comparo la arquitectura de tests actual (playwright-bdd + Gherkin en español, workers:1 por estado compartido en @aprendizaje) contra KATA (ATC, factorias de datos, trazabilidad @atc) y recomendo explicitamente no migrar ahora — ninguno de los 13 ADRs existentes documentaba esa decision. Creado `ADR-0014-testing-architecture-playwright-bdd.md`: decision + señales concretas que justificarian revisarla (segundo repo/equipo consumiendo los tests, wall-clock de CI como cuello de botella real, o necesidad de resolver el estado compartido de @aprendizaje por otra razon). Draft en `Proposed` (regla: la IA redacta, el humano acepta) — usuario acepto en la misma sesion, flipeado a `Accepted`. PR #147 contra dev. Al abrir el PR, el pipeline de CI de FRESCO-266 (pr-check.yml) no programo ningun run — ni un check-suite de github-actions aparecio, ni siquiera tras un push de sincronizacion; diagnosticado sin causa raiz identificada (no es fork, Actions habilitado, sin rulesets ni branch protection). Guardado como discovery en memoria para retomar una vez se active branch protection (paso 4 pendiente de FRESCO-266). Mergeado igual (repo:check verde en local) y propagado dev -> staging -> main (ff-only), ambos deploys de Vercel en Ready. Jira: Listo -> WIP -> Control de calidad -> Merged -> Finalizada.
- Por que: usuario pidio trabajar la 270; hallazgo de auditoria externa, decision arquitectonica sin dueño documentado.
- Siguiente: diagnosticar por que GitHub Actions no programa runs en PRs que no tocan el workflow file (posible relacion con activar branch protection); completar los pendientes de FRESCO-266 (secreto ENV_FILE, branch protection, recrear usuarios de prueba de Supabase).

## 2026-08-26 - Branch protection en GitHub (dev/staging/main)
- Que: activada proteccion de rama via `gh api` PUT branch-protection en las 3 ramas. `dev`: requiere PR (0 approvals, solo maintainer) + bloquea force-push/delete — coincide con el flujo real (feature/fix aterrizan via PR). `staging`/`main`: solo bloquea force-push/delete, SIN requerir PR — porque la promocion dev->staging->main es push directo ff-only (decision de git_strategy del 2026-08-21), exigir PR ahi habria roto ese flujo. `repo:check` de FRESCO-266 NO se marco como required status check todavia, por el bug de scheduling de GitHub Actions descubierto en FRESCO-270 (los runs no se programan de forma fiable) — exigirlo ahora habria dejado todos los merges futuros bloqueados esperando un check que nunca se crea.
- Por que: usuario pidio activar branch protection; decisiones de alcance (dev vs staging/main, sin required status check) confirmadas con el usuario antes de aplicar por el conflicto real con el flujo de promocion existente.
- Siguiente: una vez resuelto el bug de scheduling de GitHub Actions (ver entrada FRESCO-270), añadir `repo:check` como required status check en `dev` (y evaluar en staging/main).

## 2026-08-26 - Diagnostico: GitHub Actions no fallaba, tardaba en programar (~7-10min)
- Que: la ausencia de check-suite de github-actions vista en FRESCO-270 (PR #147) no era un bug real: era retraso severo de cola, no fallo de scheduling. Confirmado abriendo un PR desagable (#148, branch chore/ci-diagnostic-test): los runs que parecian "nunca disparados" del PR de FRESCO-270 (apertura ~16:47 y el push de sincronizacion ~16:51) finalmente ejecutaron a las 16:58 y 17:01 — 7-10 minutos de retraso, no ausencia. Ningun cambio de configuracion necesario; el workflow, permisos y branch protection estaban bien desde el principio. PR #148 y su branch cerrados/borrados tras confirmar el diagnostico (remote + local + refs podados).
- Por que: usuario pidio diagnosticar la causa real antes de decidir si exigir `repo:check` como required status check.
- Siguiente: al activar `repo:check` como required status check (paso pendiente de branch protection), tener en cuenta el retraso de cola — un PR puede tardar minutos en mostrar el check, no asumir que "no aparece" significa que fallo. Re-medir el retraso mas adelante para confirmar si es consistente o si baja con el tiempo.

## 2026-08-26 - Usuarios de prueba de Supabase recreados (los 4 de .env)
- Que: contrario a lo asumido en FRESCO-271 ("usuarios borrados"), los 4 usuarios de Auth (LOCAL_USER_EMAIL, PRO_TEST_USER_EMAIL, USER_EMAIL_PRE, TEST_USER_EMAIL) seguian existiendo — lo que faltaba era la fila `user_profiles` de 3 de ellos (LOCAL_USER_EMAIL ya la tenia, recreada aparte el 25 ago). Sincronizadas las 4 contraseñas via Supabase Admin API (`auth.admin.updateUserById`) para que coincidan con las credenciales que el usuario paso en el chat. Creadas las 3 filas `user_profiles` que faltaban (PRO_TEST_USER_EMAIL -> plan pro, los otros dos -> plan free). Ademas, `qa-pro-test@fresco.qa` no tenia ningun historial de menus — replicado el fixture real de `aprendizaje-pro.steps.ts` a mano (semana pasada, 21 slots reales marcados `cocinada`, luego `generate-meal-plan` real para la semana actual) para que `generacion-determinista`/`entrega-parcial`/`registro-progresivo-edge` tengan historial real que leer. Verificado: semana actual con `explicacion_aprendizaje` no nula.
- Por que: usuario paso el listado de credenciales y pidio recrearlos; el historial Pro se seguia sin sembrar tras confirmar con el usuario.
- Siguiente: con esto resuelto, evaluar flipear `test:e2e` de informativo a bloqueante en `pr-check.yml` (pendiente de FRESCO-266); considerar automatizar este seed (script en `scripts/`) para no repetirlo a mano si se vuelve a perder.

## 2026-08-26 - FRESCO-277 mergeado y propagado a Control de calidad + pre + prod
- Que: automatizados los 3 escenarios restantes de FRESCO-277 (Grupo B: checkout/portal redirect UI; Grupo C: inspeccion de la Checkout Session via API real de Stripe, con gap documentado en trial_period_days que Stripe no devuelve al leer). De paso arreglado un bug real que dejaba roto TODO Grupo A: `seedProBaseline` y el reset a Free usaban el token de usuario en vez de service_role, chocando con el trigger `protect_subscription_columns` (ADR-0007, previo a esta rama). PR #150 mergeado a dev (merge-commit), promocion ff-only dev->staging->main. Los 10 escenarios @suscripcion en verde contra dev server real antes del merge. Ticket transicionado a Control de calidad (equivalente local de "Ready For QA").
- Por que: usuario pidio cerrar el ciclo completo del ticket: merge, push y propagacion a pre y prod.
- Siguiente: QA manual sobre fresco-pre.vercel.app antes de dar la historia por completamente cerrada; considerar si Grupo C amerita cobertura adicional completando el checkout hospedado real (evaluado y descartado por ahora, ver nota en regression.feature).

## 2026-08-27 - Playwright parametrizado por entorno + STRIPE_WEBHOOK_SECRET real por env
- Que: `playwright.config.ts` ya no hardcodea `baseURL` a localhost -- lee `PLAYWRIGHT_BASE_URL` (default local), con scripts `test:e2e:staging`/`test:e2e:production`. Corriendo el suite completo contra staging por primera vez salio a la luz que `STRIPE_WEBHOOK_SECRET` (local Y el de Vercel Preview) eran placeholders, nunca un secret real de Stripe -- ningun webhook real podia verificar contra staging. Usuario genero 3 secrets reales en Stripe (`STRIPE_WEBHOOK_SECRET_PRE/DEV/PROD`) y los puso en `.env` + Vercel. `test-helpers.ts` (`resolveWebhookSecret()`) elige el secret segun `PLAYWRIGHT_BASE_URL`; production deliberadamente sin wire (no se fabrican eventos de Stripe reales contra prod). Confirmado: los 10 escenarios `@suscripcion` (Grupo A/B/C de FRESCO-277) pasan en verde contra staging real. De paso arreglado el mismo bug de clase en `aprendizaje-pro.steps.ts` (PATCH de `plan` usaba token de usuario en vez de `service_role`, chocaba con el trigger `protect_subscription_columns` de ADR-0007).
- Por que: usuario pidio confirmar el suite contra staging tras cerrar FRESCO-277; la parametrizacion desenmascaro un gap real de infra (webhooks de Stripe nunca wireados de verdad fuera de local).
- Siguiente: `tests/test-helpers.ts` y `tests/steps/aprendizaje-pro.steps.ts` estan SIN COMMITEAR en `main` (rama incorrecta, hay que mover el trabajo a una rama feature antes de commitear). Quedan 6 fallos sin arreglar corriendo contra staging: `shopping-list.steps.ts:100` (regex desactualizado, coma decimal + simbolo € en vez de "EUR") y 5 fallos de timing contra el deploy remoto (signup, generacion-menu, invitada anonima, 2 de aprendizaje) sin diagnosticar todavia -- probablemente timeouts pensados para localhost chocando con latencia real de Vercel Preview.

## 2026-08-27 - FRESCO-31 batch adicional de fotos (627->650)
- Que: corrido batch.30 de fetch-recipe-photos.ts, 24/30 hits. Aplicado via SQL directo a Supabase (jdqemhewjrjuopssdurn). Verificado cero duplicados de foto_url tras aplicar. Conteo real: 650/1000 recetas con foto.
- Por que: usuario pidio continuar FRESCO-31 (backfill de fotos via Unsplash).
- Siguiente: quedan 350 recetas sin foto; seguir corriendo batches de 30 (limite Unsplash 50 req/hora). Hit rate bajando segun se satura cada bucket combinatorio de nombre.

## 2026-08-27 - Suite e2e completo en verde contra staging (6 fallos arreglados, PR #151/#152/#153)
- Que: diagnosticados y arreglados los 6 fallos e2e contra staging. Ninguno era "timeout de Vercel" -- todo drift app<->test o fixture agotado. PR #151 (mergeado): `resolveWebhookSecret()` elige el secret de Stripe por `PLAYWRIGHT_BASE_URL` + PATCH de plan con service_role en `aprendizaje-pro` + regex de lista-compra al formato real (`57,66-78,01EUR` -> coma decimal + simbolo euro, no "EUR"). PR #152 (mergeado): `registro-progresivo-edge` clickea "Continuar como invitada" (FRESCO-197 IdentityStep, la sesion anonima ya no se crea sola) + rellena presupuesto obligatorio (FRESCO-263/265) en `registro-progresivo-edge` y `generacion-determinista`; nuevo `reseedCurrentWeekPlan()` en `aprendizaje.steps.ts` borra+regenera el plan de la semana (los 21 slots se consumian permanentemente run tras run, "queda fijado"). PR #153 (mergeado): mock de signup pasa de glob `**/auth/v1/signup` a regex -- FRESCO-264 anadio `emailRedirectTo`, supabase-js lo vuelve `?redirect_to=...` y un glob de segmento final nunca matchea una URL con query. Suite completo: 31/31 en verde contra fresco-pre.vercel.app (1.5m, antes 9.2m por los 6 timeouts de 90s).
- Por que: usuario pidio confirmar el suite contra staging de verdad tras cerrar FRESCO-277 y arreglar lo que saliera.
- Siguiente: `playwright test` NO re-ejecuta `bddgen` -- si un step cambia sus fixtures hay que correr `bddgen` antes o el spec generado pasa una lista de fixtures vieja; candidato a prepender `bddgen &&` a los scripts `test:e2e*`. Mismo bug latente de glob-vs-query en `registro-progresivo.steps.ts:62` (`page.route(**/auth/v1/user)` para `updateUser({email})`), pero ese escenario no es @automatizado; quien lo reactive necesita ademas los cambios de IdentityStep/presupuesto/cookie de #152.

## 2026-08-27 - Propagacion de la tanda de fixes e2e a los 3 entornos
...
## 2026-08-27 - Re-auditoria tecnica agresiva (2a pasada) + epica FRESCO-278 con 17 hijos de remediacion
- Que: segunda pasada de auditoria en 4 subagentes paralelos (repo/git + `.context/` + secretos · backlog + tablero de Jira via acli · suite de tests + CI · Playwright a 390px sobre `fresco-pro.vercel.app`), contrastada con el baseline de Dojo/Ely del 14-ago (4,7/5). Global ~3,7/5 (Fundacion 5,0->3,5, Backlog 5,0->3,5, Trazabilidad 5,0->4,0, Diseno 4,0->3,5, Testabilidad 4,0->3,5, Uso del agente 5,0->4,3). La caida es profundidad de inspeccion (esta vez si entro al tablero de Jira, midio contraste real, reviso branch protection) + deriva de 5 epicas de agosto, NO regresion. 3 ALTO nuevos con el mismo patron "construido pero no muerde": CI sin `required_status_checks` en ninguna rama (main/staging/dev), campos de QA estructurada de Jira al 0% board-wide (10 campos, no 2 como decia el baseline), CTA primario ambar `#df8c26` a 2,40:1 (falla WCAG AA, afecta el boton de conversion de la landing). Informe autonomo en `scratchpad/audit2/reauditoria-fresco.html` (+ copia en `~/Downloads/`); hallazgos por frente en `scratchpad/audit2/{A,B,C,D}-*.md`; baseline en `audit2/baseline-audit-2026-08-14.txt`. El publish como artifact quedo bloqueado por el clasificador de auto-mode (no es permiso aprobable en chat). Creadas epica FRESCO-278 ("Re-auditoria tecnica agosto 2026 - remediacion", prioridad Medium por defecto de epicas) + 17 hijos FRESCO-279..295, todos etiqueta `auditoria-2`, prioridad por severidad (5 High / 7 Medium / 5 Low), 4 links `Relates` a FRESCO-266/267/270. Nada transicionado (todas en `Listo`).
- Por que: el usuario pidio repetir la auditoria agresiva del baseline para ver que se habia corregido en 13 dias, y luego crear las Jiras con todo lo que hay que solucionar.
- Siguiente: accionar por retorno - (1) activar `required_status_checks` en main/staging/dev (~10 min, FRESCO-279, mayor retorno de todo), (2) cablear unit tests + aterrizar FRESCO-266/PR#149 mergeando dev + re-run (FRESCO-280), (3) rellenar Severity/Root Cause/Error Type/Story Points en trabajo nuevo (FRESCO-281), (4) arreglar el token ambar (FRESCO-283). Ver `project = FRESCO AND labels = auditoria-2 ORDER BY key`. Gotcha guardado: `acli create --from-json` rechaza `priority` y `parent` como claves JSON, hay que ponerlas post-create por REST PUT + GET de verificacion.

## 2026-08-27 - FRESCO-296 boton "Atras" del onboarding sale a la landing
- Que: en el paso 1 del wizard de onboarding el boton "Atras" estaba `disabled` (callejon sin salida). Ahora en el paso 1 navega a la landing (`router.push("/")`); pasos 2-4 sin cambios. 1 archivo (`app/onboarding/page.tsx`), 1 handler. Verificado live-UI en dev y en `fresco-pro.vercel.app`: paso 1 -> `/`, paso 2 -> paso 1. types+lint limpios, tests onboarding 21/21.
- Por que: defecto FRESCO-296 (Medium) reportado por el usuario; expectativa UX de que "Atras" en el primer paso salga del flujo.
- Siguiente: nada pendiente. PR #155 -> dev -> staging -> main (ff-only), los 3 deploys Vercel READY en commit 5b909b2, FRESCO-296 en Finalizada. Esta tanda incluye tambien el commit del entry de bitacora de la re-auditoria/FRESCO-278 que habia quedado sin commitear de una sesion previa.

## 2026-08-27 - FRESCO-297 CORS bloqueaba las Edge Functions desde fresco-dev + banner enganoso en /menu sin plan
- Que: causa raiz del "no pudimos conectar con el servidor" al generar menu: `supabase/functions/_shared/cors.ts` `ALLOWED_ORIGINS` no tenia `https://fresco-dev.vercel.app` (el upgrade a 3-tier del 21-ago lo dejo fuera). Preflight OPTIONS 200 sin cabecera `Access-Control-Allow-Origin` -> el navegador bloquea el POST real -> `TypeError`. Confirmado en logs (14:13-14:14: solo OPTIONS, cero POST) y reproducido con curl. Fix: +fresco-dev, -fresco-staging (alias stale). 6 Edge Functions redeployadas (`--use-api`): generate-meal-plan, generate-shopping-list, update-recipe-status, reassign-guest-data, delete-account, get-shopping-list-suggestions. Proyecto Supabase unico -> el fix entra en los 3 entornos a la vez. Segundo problema: `/menu` sin plan mostraba `CalendarSuggestionBanner` ("Ver mi plan semanal"), enganoso; `(app)/layout.tsx` redirige por existencia de perfil, no de plan. Fix: quitar el banner de esa rama y poner `NoMenuEmptyState` primero.
- Por que: defecto FRESCO-297 (Medium). Usuario habia intentado generar menu en fresco-dev, error, y luego "estaba dentro sin menu" sin poder generarlo.
- Siguiente: nada pendiente de la 297. PR #156 -> dev -> staging -> main (ff-only, commit 063e5ef), Edge Functions ya en prod, FRESCO-297 en Finalizada. Follow-ups anotados en la Jira: (1) anadir bloque `dev:` a `.agents/project.yaml` (la URL de dev no tiene sitio canonico, por eso cors.ts se desincronizo), (2) mover ALLOWED_ORIGINS a un secret de Edge Function.

## 2026-08-27 - FRESCO-281 campos de QA estructurada horneados en el flujo de skills
- Que: hallazgo 03 (ALTO) de la re-auditoria - Severity/Root Cause/Error Type/Story Points al 0% board-wide. Solucion en tooling de agente (docs de skill), no codigo de app. Story Points: se escribe en `/sprint-development` Stage 1, estimacion del equipo dev desde el desglose de esfuerzo del plan (`story-plan.md` seccion Estimated Effort + slug de escritura + checklist; `SKILL.md` parrafo Stage 1 + gotcha #13 + pre-flight). Severity + Error Type: backfill en el intake de bug si vienen vacios + confirmacion al cerrar (`bug-fix-workflow.md` Fase 1 Step 2b nuevo, Fase 7 Step 1, Output Checklist); Root Cause al cerrar ya estaba. Conflicto resuelto: el hallazgo pedia SP al refinar la historia, choca con el anti-pattern I16 de `/product-management` (el PO/BA no estima); movido a Stage 1 de dev satisface auditoria + respeta I16, con cross-refs en ambas skills (`product-management/SKILL.md` I16 + checklist, `story-refinement.md` Ready checklist). Sin retro-fill de los ~220 items cerrados. ATP/ATR fuera de alcance (el ticket los aplaza al repo de QA). Pre-commit verde: tsc, lint-vars (0 errores, refs `{{jira.*}}` validadas), lint-skills (0 errores en las 2 skills tocadas).
- Por que: defecto FRESCO-281 (Tarea, label auditoria-2, epica FRESCO-278). Sin campos de QA consultables no hay velocity ni densidad de defectos por feature para un proyecto cuya tesis es shift-left QA.
- Siguiente: nada pendiente. PR #157 -> `dev` (merge commit 1b472b7), FRESCO-281 en Finalizada. No promocionado a staging/main - es tooling de agente, entra cuando promocione dev normalmente. El efecto real se vera al ejecutar la proxima historia por `/sprint-development` (debe escribir story_points) y el proximo bug (debe setear severity + error_type en el intake).

## 2026-08-27 - FRESCO-298 iconos lucide a stroke-width 2 (regla CSS global eliminada)
- Que: `app/globals.css` tenia una regla global `svg.lucide { stroke-width: 3 }` que forzaba grosor 3 en TODOS los iconos lucide-react; el stroke-width de CSS gana sobre el atributo strokeWidth del SVG, asi que hasta los componentes que ya pasaban strokeWidth={2} (sidebar, bottom-tab-bar, landing) se pintaban a 3, y los stat cards de /menu heredaban el 3 sin prop. Contradecia DESIGN.md:293 (set nav/UI a 2px; el bump a 3px fue mala atribucion a FRESCO-85/86/87, que unificaron TAMANO de glifo no grosor). Fix: eliminada la regla + comentario, y el safelist [lucide] de tailwind.config.ts que solo existia para esa regla. Checks <=16px mantienen su strokeWidth={3} por prop. 2 ficheros, 20 borrados. Verificado: types/lint/format/build verde, CI repo:check verde, live-UI en dev + prod (computed 2px, antes 3px).
- Por que: defecto FRESCO-298 (Error, Low, label design-fidelity) reportado por el usuario con capturas de los tres entornos.
- Siguiente: nada pendiente. PR #158 -> dev (merge 4f25166), ff-only dev->staging->main, 3 deploys Vercel READY en 4f25166, 2px verificado en fresco-pro. FRESCO-298 Finalizada. Custom fields de bug van como comentario: el tipo Error de este workspace no los expone en pantalla.

## 2026-08-27 - FRESCO-283: contraste WCAG AA en CTAs ámbar
