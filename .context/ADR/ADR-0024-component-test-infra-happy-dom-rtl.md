...
1. **One preload**, `bunfig.toml` → `preload = ["./bun-test-setup.ts"]`. Its first statement is `GlobalRegistrator.register({ url: 'https://test.fresco.local/' })`. `@testing-library/*` binds `document.body` at import-eval time and ESM `import` hoists above executable code, so testing-library is pulled in with `require()` **after** the register call, not a top-level import. A guard throws loudly if `window` is still undefined after `register()`.

   > **OPEN — CI blocker (2026-09-03).** This works locally (macOS, bun 1.3.14: 458/458 green, ~1s) but the GitHub Actions runner (Linux, same bun via `setup-bun@v2`) throws `ReferenceError: window is not defined` from `react-dom` at test time. The preload *does* run there (its env-dummy lines make `edge-functions.test.ts` pass) and the guard does *not* fire (so `register()` set `window`), yet `window` is gone by the time test files execute — as if the preload's realm and the test-file realm diverge on the runner. An in-band variant (register from a side-effect module imported first by `component-render.tsx`) did not reliably run before `@testing-library/dom` either. Needs a Linux repro to settle; until then the component-test job is red. Candidate directions: a `bun test --preload` CLI flag in the workflow, a bun version bump, or jsdom.
