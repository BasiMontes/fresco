...
1. **One preload**, `bunfig.toml` → `preload = ["./bun-test-setup.ts"]`. Its first statement is `GlobalRegistrator.register({ url: 'https://test.fresco.local/' })`. `@testing-library/*` binds `document.body` at import-eval time and ESM `import` hoists above executable code, so testing-library is pulled in with `require()` **after** the register call, not a top-level import.
2. **A `beforeEach` re-asserts the DOM.** On the GitHub Actions runner the preload's global `window` did **not** survive to test-file execution (`ReferenceError: window is not defined` from `react-dom`), though it did locally and in a Linux Docker container with the same bun — the preload ran (its env dummies made `edge-functions.test.ts` pass) but `window` was gone by test time. So `bun-test-setup.ts` also runs `ensureHappyDom()` in a global `beforeEach`: if `window` is missing it `unregister()`s (the static "registered" flag can outlive the global) and re-registers. This makes the DOM deterministic regardless of what bun does to globals between the preload and each file.
3. **`tests/component-render.tsx` exports its own `screen`** — a `Proxy` that re-derives `within(document.body)` on every access. `@testing-library/dom`'s stock `screen` binds `document.body` once at import; a `beforeEach` re-register swaps `document`, so the stock `screen` would point at a detached body.
4. **The DOM is global, not per-file.** Every test file gets `window`/`document`. The ~415 pure-logic tests do not read them; the cost is one sub-millisecond registration and ~250ms of one-time module init across the run — the suite stays ~1s locally, far under 30s.
5. **`url` is set** so `window.location` has a real origin: `lib` code that branches on `typeof window` now takes the browser path under test, matching the real app rather than an SSR shim.
6. **`@testing-library/jest-dom` self-registers** its matchers on `bun:test`'s `expect`. It ships no types for that runner, so `tests/jest-dom.d.ts` augments `bun:test`'s `Matchers` interface.
7. **RTL `cleanup` runs in a global `afterEach`** in `bun-test-setup.ts` — `bun test` wires none of its own, so without it rendered trees accumulate in the shared happy-dom document across cases.
8. **`@/lib/supabase/client` is stubbed once, globally**, in `bun-test-setup.ts`. It pulls the whole `@supabase/ssr` + `@supabase/supabase-js` graph; a per-file `mock.module` on it (or on any `@/lib/api/*` module that re-exports through it) makes bun re-transpile that graph per file and turns a ~1s suite into a ~75s one. Component tests that must assert on a data call use `spyOn` on the specific `@/lib/api/*` function, or are written against the inert stub's "no session → throws" path.
9. **Shared entry point:** `tests/component-render.tsx` exposes `renderWithProviders` (today a pass-through — `PostHogProvider` is already stubbed — kept so a future provider is a one-line change) + `setupUser`. `tests/mocks/next-navigation.ts` is the shared `useRouter`/`usePathname` stub.
10. **Scope invariant (from FRESCO-408):** only components with real risk get tests — state logic, form validation, destructive actions, conditional rendering, or high-reuse design-system primitives. Purely presentational components are deliberately left uncovered; the PR that adds a batch documents which were chosen and why.
11. **`Dialog`-based components — the real spin cause (FRESCO-419).** FRESCO-409 deferred `Dialog`-mounting components blaming `components/ui/dialog.tsx`'s nested double `requestAnimationFrame`. That was a **misdiagnosis** — the spin was the broken intermediate FRESCO-409 setup (the `mock.module` + `import * as real` re-transpile cliff, plus half-registered DOM). With the infra consolidated, `Dialog` renders in ~300ms–1s. The actual remaining trap is **`waitFor` / `findBy` polling on a condition that never resolves under happy-dom** — e.g. asserting that `Dialog` has *unmounted* after its 150 ms CSS close transition: happy-dom's `setTimeout` for that transition does not advance inside a `waitFor` loop, so `waitFor` busy-spins to ~73 s. Rules for `Dialog` tests: assert the immediate `is-closing` class, not the eventual unmount; use `findBy*` only for DOM that *does* appear; keep async submit/delete handlers (which depend on the shared inert Supabase client and are cross-file-flaky) in e2e. FRESCO-419 shipped `delete-week-button`, `delete-account-dialog`, `create-recipe-form` tests on that basis — no rAF shim needed (a synchronous rAF breaks `dialog.tsx`'s self-referential `rafId` read; a microtask rAF re-introduces the spin).
