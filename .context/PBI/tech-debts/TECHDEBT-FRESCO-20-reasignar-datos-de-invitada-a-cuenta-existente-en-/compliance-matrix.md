...
| `/signup` conflict UI (password prompt + button) | manual:live-ui-validation | Full click-through: fresh guest → real onboarding → real generation (same week as the existing account, forcing the conflict) → conflict card renders → password field + disabled-until-filled button → clicked with the real password → redirected to `/menu` as the real account. DB re-verified clean after (1 plan, no duplication). | covered |
