# Biblioteca | View full recipe detail

**Jira Key:** [FRESCO-69](https://basiliomontescastano.atlassian.net/browse/FRESCO-69)
**Epic:** [FRESCO-64](https://basiliomontescastano.atlassian.net/browse/FRESCO-64) (Biblioteca de Recetas)
**Type:** Historia
**Status:** Listo
**Priority:** Medium
**Story Points:** -

---

## Overview

## User story

As Laura, I want to open a recipe from the Biblioteca and see its full detail, so that I can decide whether to cook it before committing to it.

## Definition of done

- [ ] Recipe detail view built, handling both catalog and personal recipes
- [ ] Verified live (Playwright)


---

## Traceability

### Historias (2)

- [FRESCO-65](https://basiliomontescastano.atlassian.net/browse/FRESCO-65): Biblioteca | Buscar recetas por nombre o ingrediente _(Finalizada)_
- [FRESCO-68](https://basiliomontescastano.atlassian.net/browse/FRESCO-68): Biblioteca | Crear una receta propia _(Finalizada)_

---

## Metadata

- **Created:** 8/3/2026
- **Updated:** 8/3/2026
- **Reporter:** Basi Montes
- **Assignee:** Basi Montes
- **Labels:** feature-extension, post-mvp

---

_Synced from Jira by sync-jira-issues_
