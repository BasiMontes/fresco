# EPIC: Control del Menú Semanal

**Jira Key:** [FRESCO-60](https://basiliomontescastano.atlassian.net/browse/FRESCO-60)
**Priority:** Medium
**Status:** Listo
**Total Story Points:** 0

---

## Description

Extiende /calendar con control directo sobre el menú de cada semana: navegar entre semanas, limpiar el plan sugerido de la semana vista y generar uno nuevo — sin depender de volver a /onboarding. El backend (generate-meal-plan) ya acepta semana_iso como parámetro explícito y ya bloquea regenerar sobre una semana con plan existente; esta épica es principalmente wiring de frontend sobre capacidad de backend ya existente.

---

## User Stories

| Key | Story | Points | Priority | Status |
| --- | ----- | ------ | -------- | ------ |
| [FRESCO-61](https://basiliomontescastano.atlassian.net/browse/FRESCO-61) | Calendario | Navegar entre semanas del menú | - | Medium | Listo |
| [FRESCO-62](https://basiliomontescastano.atlassian.net/browse/FRESCO-62) | Calendario | Eliminar el menú de la semana actual | - | Medium | Listo |
| [FRESCO-63](https://basiliomontescastano.atlassian.net/browse/FRESCO-63) | Calendario | Generar un menú nuevo para la semana | - | Medium | Listo |

---

## Metadata

- **Created:** 8/3/2026
- **Updated:** 8/3/2026
- **Reporter:** Basi Montes
- **Assignee:** Basi Montes
- **Labels:** new-feature, post-mvp

---

_Synced from Jira by sync-jira-issues_
