...
| `@smoke` | A minimal 5-6 scenario subset of `@automatizado` happy paths, run post-deploy against the live deployment URL by `.github/workflows/post-deploy-smoke.yml` (`bun run test:e2e:smoke`). Add only to fast, low-flake, high-signal scenarios that already pass in CI; keep the set small |
