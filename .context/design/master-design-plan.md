...
- **Tokens.** `recipe-card` image treatment — `aspect-[4/3]` + `.recipe-photo` grade + `RecipePlaceholder`,
  identical to the cards (FRESCO-447, was `aspect-video` + bare icon — §5-S); `tag`/`tag-accent`/`tag-accent-2`
  (allergen flags on accent-2); `button-icon` circular heart; body copy at 15px base (never smaller for steps).
- **Components.** `components/recipes/recipe-detail.tsx`, `components/recipe/favorite-toggle-button.tsx`,
  `components/recipe/recipe-placeholder.tsx`.
...
| §5-J | Calendar (`/calendar`) — Learning bridge card | New surface: `components/calendar/learning-bridge-card.tsx`. Replaces `CalendarGrid`'s flat Free notice with a `card` (`pro` variant — **not** `insight`, which §5-D reserves for the real payoff) stating the real learning mechanism (no-repeat 2 wks, boost cooked, sink discarded) + a worked example + a `/profile` CTA for Free. Shown to Free always; to Pro only while no slot is marked yet (week-1 bridge). | No mockup — novel surface (A4-H12). | Built DESIGN.md-first per Critical Rule 14 approach A: existing `card` tokens + live copy patterns, no new tokens. The 3 mechanism bullets are verified against `generate-meal-plan/{index,menu-selector}.ts`, not invented. | bitácora 2026-09-01 (FRESCO-369) |
| §5-K | Motion layer (`/(app)/*`) — page transitions | Shipped as a **fade-in of the incoming view** (`components/layout/page-transition.tsx`, keyed by `usePathname()`), not the true outgoing+incoming cross-fade the FRESCO-245 AC scenario 1 describes. Reduced-motion and browser back/forward are covered. | FRESCO-245 AC ("la vista saliente y la entrante hacen un cross-fade") | The only SPA mechanism for a real cross-fade in Next 16 is `experimental.viewTransition`; enabling it raced the calendar drag-and-drop optimistic update and broke the reorder e2e (STORY-FRESCO-11). Fade-in delivers the "no hard cut" goal with zero risk to shipped flows. Revisit when the View Transitions API is stable in Next. | bitácora 2026-09-02 (FRESCO-245) |
| §5-L | Motion layer — list/card animation (FRESCO-246) | (a) **Exit animation** (AC scenario 2 — fade-out + collapse before DOM removal) not built: no in-place item-removal flow exists in the live UI (the shopping list toggles items to "comprado" and keeps them in the DOM; deleting a personal recipe happens on the detail page then `router.push('/recipes')`; the calendar has 21 fixed slots). (b) **Calendar slots excluded** from the enter/stagger animation: windowed grid remounts slots on day pagination (stagger would replay), no add flow, and a `transform` on the drag drop-target risks dnd-kit measurement (§5-K lesson). Enter + stagger + reduced-motion shipped for the shopping list and recipe library only. | FRESCO-246 AC (scenario 2; "platos del calendario" in scenario 1) | Building against the live UI per Rule 14 — the removal flows and a stable calendar list the AC assumes do not exist. Revisit (a) if an in-place delete ships; (b) is unlikely to be worth the drag risk. | bitácora 2026-09-02 (FRESCO-246) |
| §5-M | Recipe card (`/menu`, `/calendar`, `/recipes`, `/favorites`) | Photo area is **full-bleed** — flush to the card's top/left/right edges, rounding its own top corners to `rounded-card` (20px) — rather than an inset image area carrying its own `rounded-image` (16px). No-photo state is `RecipePlaceholder` (`components/recipe/recipe-placeholder.tsx`): a monochrome `neutral-*` gradient keyed to `categoria` (`lib/recipes/category-gradient.ts`) + the recipe's initial in Fraunces. The calendar `SlotCell` now consumes the same shared `RecipeCardMedia` instead of a hand-mirrored image block. | `DESIGN.md` §Components `recipe-card` ("full-width image area at the top, `rounded.image`") | Full-bleed reads more editorial ("de simplona a premium editorial" — the epic's thesis) and the 4px radius delta is imperceptible; the gradient stays in the neutral ramp to respect one-accent discipline (FRESCO-440). No new tokens. | FRESCO-441 story plan (2026-09-06) |
| §5-N | Form fields (`input`, `dropdown` trigger) — `/login`, `/signup`, `/onboarding`, `/profile`, `/forgot-password`, `/update-password` | Resting **border** is `{colors.neutral-600}` (1px), not DESIGN.md v2's `{colors.border}` (text @ 16%) hairline. Width stays 1px ("hairline" = 1px, per the FRESCO-443 scope bullet). | `DESIGN.md` §Components `input` ("`{colors.border}` outline at rest") | The 16%-opacity hairline measures ~1.2:1 on the cream page (`#FAF3E3`) — the FRESCO-443 AC ("cumple contraste AA … ≥3:1 UI") and WCAG 2.2 SC 1.4.11 require ≥3:1 for a UI-component boundary. `neutral-600` is the lightest neutral-ramp token that clears 3:1 on page (~4.2:1), card (~4.3:1) and the field's own `surface` fill (~3.6:1). `DESIGN.md` §Components `input` updated to match — this is now the contract. | FRESCO-443 story review (2026-09-06) |
...
| §5-V | App shell / navigation (`/(app)/*`) — sidebar account popover (FRESCO-514) | New capability, no committed mockup (2 reference screenshots reviewed in conversation, never saved as project files) — built LIVE-UI-FIRST against the live `sidebar-account` footer + `DESIGN.md` tokens. New `components/ui/popover.tsx` panel uses `z-[1000]` ("modal" band) rather than the literal `dropdown: 100` `DESIGN.md` assigns to in-flow listbox panels like `Dropdown`. | `DESIGN.md` Elevation & Depth — "Z-index layer convention... dropdown `100`, modal `1000`" | This panel is anchored (not portalled) at the very edge of the sidebar's own `overflow-y-auto` `<aside>`; at `z-100` it risks being clipped/occluded by sibling scroll content the way `Dialog`'s reserved `z-1000` band does not. Flagged as a one-line divergence rather than silently deviating. | FRESCO-514 story plan (2026-09-15) |
...
| FRESCO-444 | Spatial rhythm & density | EPIC-FRESCO-436 Visual redesign | 4.8 Calendar | 4.3 Signup, 4.6 Onboarding, 4.9 Shopping list, 4.7 Home, 4.17 App shell |
| FRESCO-445 | Editorial signature: footer wordmark + landing hero | EPIC-FRESCO-436 Visual redesign | 4.1 Landing | — |
