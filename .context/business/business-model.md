...
Receipt scanning, pantry/inventory management, aggressive "morning briefing" notifications, voice operations, background/wake-lock-style device APIs, complex local-first architecture, price learning, leftover-ingredient AI, batch-cooking mode, expiration alerts, complex glassmorphism UI, gamification, community/social features, a recipe marketplace, a native app before 1,000 paying users, and B2B before B2C is validated.
...
**Scope reversal (2026-09-10, founder-approved — `ADR-0027`):** "supermarket integrations" and "price comparison" are **removed** from this blacklist. Epic FRESCO-332 with FRESCO-345 (list → supermarket) and FRESCO-346 (price comparison), plus their zero-cost enabling pieces (shopping-list export, the FRESCO-488 ingredient→product layer), move to the active backlog. **Bounded by the standing no-extra-spend constraint:** buildable now = structured export + affiliate deep links (Awin); still gated (on budget, not on the retention metric) = any recipe-commerce provider integration (Northfork / Whisk) or other paid dependency. The AI's recorded recommendation was to wait for the MRR/retention gate; the founder decided to proceed — `ADR-0027` holds the full context, the way EPIC-FRESCO-227 did for self-serve payment. The moat invariant (`ADR-0001`) is unchanged: this is an added-value layer, never the Free/Pro boundary, and stays behind concierge-validation work in priority order. Feasibility evidence: `scripts/spikes/fresco-345-grocery-deeplink/`. JSON/CSV export is off the blacklist **only** as the shopping-list export feature above.
