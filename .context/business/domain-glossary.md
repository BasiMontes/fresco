...
| **ADR** | Architecture Decision Record | An append-only record of a hard-to-reverse architecture decision, stored under `.context/ADR/`. This project has 13 (`ADR-0001`–`ADR-0014`; `ADR-0010` was never issued). `ADR-0001-behavioral-learning-moat.md` formalizes the Free/Pro data-model split; the rest cover guest auth, deterministic menu selection, Stripe, Sentry, PostHog, pg_cron, web-push, and the testing architecture. |
