...
/**
 * `client.rpc(...)`'s function-name generic is constrained to
 * `keyof Database['public']['Functions']`, generated by `bun run db:types`
 * from the LIVE remote schema. `swap_meal_plan_slots` (this batch's new
 * migration) is intentionally NOT applied to any remote/production project
 * yet — per this batch's own scope, migrations are reviewed before being
 * applied — so `lib/supabase/types.ts` (do-not-hand-edit, silently
 * discarded on next `db:types` run) has no entry for it yet. This narrow,
 * explicit cast is the escape hatch; remove it once the migration is
 * applied and `bun run db:types` regenerates the real entry.
 */
interface SwapMealPlanSlotsClient {
  rpc: (
    fn: 'swap_meal_plan_slots',
...
export async function swapMealPlanSlots(
  ...
  const { error } = await (client as unknown as SwapMealPlanSlotsClient).rpc('swap_meal_plan_slots', {
    p_slot_a_id: slotAId,
    p_slot_b_id: slotBId,
  });
